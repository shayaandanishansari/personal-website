 ...
 * Output pages have all content baked in — no runtime rendering JS needed.
 */
...
function buildLanding() {
  ...
  const cardsHtml = CARDS.map(({ category, position, label }) => {
    ...
    if (category === 'apps') {
      ...
      if (rest.length) inner += `<div class="projects-grid">${rest.map(p => projectCard(p)).join('')}</div>`;
    } else {
      inner = projects.map(p => projectCard(p)).join('');
    }
    return `<div class="category-card" data-position="${position}" data-category="${category}">
...
function buildProjectsList() {
  const entries = PROJECTS.map(p => `<a class="project-entry" href="${p.id}.html" data-category="${p.category}">
